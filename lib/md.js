...
/**
 * Un href qu'on accepte de poser dans le document.
 *
 * La doc du produit est écrite par nous ; la sortie d'un MODÈLE ne l'est pas. Un
 * `[clic](javascript:…)` rendu tel quel donnerait à un texte généré le pouvoir d'exécuter
 * du code dans la page. Le contenu du lien reste visible — seul sa destination devient
 * inerte, ce qui se voit et ne cache rien.
 */
export const lienSur = (href) => {
  const h = String(href ?? '').trim();
  return /^(https?:\/\/|mailto:|#|\.{0,2}\/)/i.test(h) ? h : '#';
};

/**
 * Ce texte gagne-t-il à être RENDU plutôt que montré tel quel ?
 *
 * Une sortie de modèle en Markdown affichée en chasse fixe se lit mal : on voit `##` et
 * `**` au lieu de titres et de gras. Mais tout n'est pas du Markdown — du JSON reste du
 * JSON, et le « rendre » en effacerait la structure au lieu de l'éclairer.
 */
export function ressembleADuMarkdown(texte) {
  const t = String(texte ?? '').trim();
  if (!t) return false;

  // Du JSON, même entouré de ```json, n'est pas du Markdown à rendre.
  const nu = t.replace(/^```(?:json)?\s*/, '').replace(/\s*```$/, '');
  try { JSON.parse(nu); return false; } catch { /* pas du JSON : on continue */ }

  return /^#{1,6}\s+\S/m.test(t)          // un titre
      || /^\s*[-*+]\s+\S/m.test(t)        // une puce
      || /^\s*\d+\.\s+\S/m.test(t)       // une liste numérotée
      || /\*\*[^*\n]+\*\*/.test(t)        // du gras
      || /^\s*\|.+\|\s*$/m.test(t);       // un tableau
}
...
export default { rendre, enLigne, echapper, plan, ancre, titre, lienSur,
                 ressembleADuMarkdown };
