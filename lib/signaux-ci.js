 ...
 */
import { CAVIARDE, caviarder } from './signaux-securite.js';
...
export const SIGNAUX_CI = {
  job_en_echec: {
    ...
    mots: ['casse', 'plante', 'echoue', 'rouge', 'erreur'],
...
/*
 * Le caviardage a DÉMÉNAGÉ dans `signaux-securite.js`, à côté des motifs.
 *
 * Il était écrit ici parce qu'un log de pipeline est l'endroit où les secrets fuient le
 * plus facilement — un `echo` de débogage, un `curl -v` qui affiche ses en-têtes. C'est
 * vrai, et c'est bien pour ça qu'il existe. Mais du coup il ne protégeait QUE le log : le
 * code, lui, partait en clair — un `.env` ouvert dans le sélecteur de fichiers, un
 * `application.yml` avec son mot de passe, une clé privée collée dans le champ.
 *
 * Il sert désormais au dernier garde-fou avant l'appel, pour TOUS les agents. On le
 * réexporte pour ne rien casser de ce qui l'importait d'ici : même fonction, même
 * comportement, à un endroit qui ne parle plus de CI.
 */
export { CAVIARDE, caviarder };
