...
export const SIGNAUX_CODE = {
  ...
  code_du_depot: {
    ...
    source: 'js/secrets-scanner.js',
    /*
     * DEUX RÉGLAGES, TOUS DEUX FACULTATIFS — et leur absence est une VALEUR.
     *
     * Pas de dossier : tout le dépôt. Pas de branche : celle par défaut. Les exiger
     * obligerait à choisir là où la question porte le plus souvent sur l'ensemble, et
     * les tester comme des manques bloquerait le cas courant.
     *
     * ── POURQUOI LA BRANCHE A ÉTÉ AJOUTÉE ────────────────────────────────────
     *
     * Le signal lisait la branche par défaut, sans moyen de dire autre chose. Sur la
     * question « analyse le code de MA branche », il rendait donc le code de `main` —
     * une réponse juste à une autre question, ce qui est pire qu'un refus.
     *
     * À ne pas confondre avec `code_de_la_branche`, qui lit ce qu'une branche a CHANGÉ.
     * Deux questions : « qu'est-ce que ce travail modifie » et « à quoi ressemble le code
     * quand on est dessus ». Sur une branche sans diff, la première n'a rien à montrer et
     * la seconde a tout le dépôt — c'est exactement le cas qui a fait apparaître ce
     * réglage.
     */
    reglages: [
...
export const HORS_SOURCE = /(?:^|\/)(?:node_modules|vendor|dist|build|target|coverage|\.git|out|\.next|\.nuxt|\.cache|__pycache__|\.venv|venv|overlays)(?:\/|$)/;

/*
 * CE QUI N'EST PAS DU CODE — écarté, compté, et dit.
 *
 * ── LE CONSTAT QUI A PRODUIT CETTE LISTE ────────────────────────────────────
 *
 * Le 2026-08-20, sur un vrai dépôt de chaîne CI : les douze premiers fichiers servis
 * étaient deux README, sept réglages d'outillage et un CONTRIBUTING de 526 lignes. Mille
 * lignes de budget dépensées AVANT d'ouvrir un seul job — c'est-à-dire avant le seul
 * contenu qu'on venait lire. Le verdict de l'usage a été sans appel : « supprime aussi
 * tous les md et fichiers obligatoires, on s'en fout, je veux juste le code ».
 *
 * Trois familles, et aucune ne décrit ce que le système FAIT :
 *
 *   les RÉGLAGES D'OUTILLAGE   `.editorconfig` configure un éditeur, `.yamllint.yml` un
 *                              linter ;
 *   la DOCUMENTATION           elle raconte le code, elle ne l'est pas ;
 *   les FICHIERS RITUELS       LICENSE, CHANGELOG, CODEOWNERS, gabarits de MR — présents
 *                              dans tout dépôt, propres à aucun.
 *
 * ── CE QUI N'EN EST PAS, ET C'EST LA PRÉCAUTION QUI COMPTE ─────────────────
 *
 * `.gitlab-ci.yml`, `Dockerfile`, `Makefile` : sur un dépôt d'outillage, c'est LE code.
 * La liste ne nomme donc que ce qui règle un éditeur ou un style, jamais ce qui décrit
 * une exécution. Se tromper ici coûterait cher et en silence — un dépôt de CI ressortirait
 * vide.
 *
 * Rien n'est perdu : tout ce qui tombe ici est compté dans `hors` et annoncé dans la
 * matière. Écarté n'est pas ignoré.
 */
export const PAS_DU_CODE = new RegExp([
  // réglages d'outillage — les points-fichiers de configuration d'éditeur et de style
  '(?:^|/)\\.(?:editorconfig|gitattributes|gitignore|gitmodules|npmrc|nvmrc|shellcheckrc',
  '|dockerignore|prettierrc[^/]*|eslintrc[^/]*|yamllint[^/]*|pre-commit-config[^/]*',
  '|husky[^/]*|gitlab-ci-local[^/]*)$',
  // documentation
  '|\\.(?:md|markdown|rst|adoc|txt)$',
  // fichiers rituels de dépôt, quelle que soit leur extension
  '|(?:^|/)(?:LICEN[CS]E|COPYING|NOTICE|CHANGELOG|CONTRIBUTING|CODE_OF_CONDUCT',
  '|SECURITY|AUTHORS|MAINTAINERS|CODEOWNERS)[^/]*$',
  // gabarits d'issue et de merge request
  '|(?:^|/)(?:merge_request_templates|issue_templates|ISSUE_TEMPLATE|PULL_REQUEST_TEMPLATE)(?:/|$)'
].join(''), 'i');
 ...
 *
 * ── POURQUOI CE PLAFOND EST HAUT, ET POURQUOI IL L'EST DEVENU ───────────────
 *
 * Il valait 40. Sur un vrai dépôt de chaîne CI — 40 fichiers pile — il ne coupait rien
 * par chance, et le suivant aurait été amputé sans qu'on comprenne pourquoi. Le verdict
 * de l'usage a été net : « soit il vérifie tout, soit il ne sert à rien ; les jetons ne
 * coûtent rien ». C'est juste. Un rapport d'architecture bâti sur un tiers du dépôt ne
 * vaut pas le tiers d'un rapport : il vaut zéro, et il coûte la confiance qu'on lui fait.
 *
 * Le plafond reste néanmoins un plafond, parce que la contrainte réelle n'est pas le prix
 * mais la FENÊTRE DE CONTEXTE du modèle : au-delà, le fournisseur refuse l'appel ou coupe
 * lui-même — et une coupe qu'on ne fait pas soi-même est une coupe qu'on ne peut pas
 * annoncer. Ce qui est écarté ici est compté et dit ; ce que le fournisseur tronquerait
 * ne le serait pas.
 */
export const MAX_FICHIERS_DEPOT = 300;

/*
 * Le budget de lignes, tous fichiers confondus.
 *
 * Il valait 2 500. Sur un vrai dépôt de chaîne CI de 7 703 lignes, il en coupait 5 203 —
 * plus des deux tiers absents d'un rapport qui prétendait décrire le dépôt. Relevé à
 * 20 000 le 2026-08-20 : à cette échelle, un dépôt d'outillage ou de configuration passe
 * entier, et c'est le seul état où l'agent sert à quelque chose.
 *
 * Ce qui le borne encore n'est pas le prix — quatorze analyses complètes ont coûté dix
 * centimes — mais la fenêtre de contexte du modèle. Voir `MAX_FICHIERS_DEPOT`.
 */
export const MAX_LIGNES_DEPOT = 20_000;
...
export function fichiersARetenir(chemins = [], { dossier = '', max = MAX_FICHIERS_DEPOT } = {}) {
  ...
  const candidats = duDossier.filter((c) =>
    !ILLISIBLE.test(c) && !HORS_SOURCE.test(c) && !PAS_DU_CODE.test(c));
...
export function carteDesZones(chemins = [], { dossier = '' } = {}) {
  ...
  for (const c of chemins) {
    ...
    z.fichiers += 1;
    if (!ILLISIBLE.test(c) && !HORS_SOURCE.test(c) && !PAS_DU_CODE.test(c)) z.source += 1;
    zones.set(zone, z);
  }
 ...
 */
/*
 * ── « ILLISIBLE » N'EST PAS UNE CAUSE, C'EST UN AVEU ─────────────────────────
 *
 * Vu à l'écran, sur un vrai dépôt : six fichiers annoncés « illisible(s) » — deux YAML de
 * jobs, un `.ts`, un `.js`, un `.py`, un `.yml`. Tous parfaitement lisibles : la MÊME
 * lecture, vingt-sept minutes plus tôt sur la même branche, les avait tous les vingt-sept.
 * La forge avait simplement refusé de les servir ce jour-là.
 *
 * Le mot décide du geste. Qui lit « illisible » range le fichier au rayon binaire et ne
 * relance jamais ; qui lit « la forge a refusé » relance et obtient le fichier. Un mot qui
 * fait renoncer à tort coûte plus cher qu'une ligne manquante.
 *
 * `nonLus` accepte donc deux formes : un chemin nu quand l'appelant ne sait pas pourquoi,
 * ou `{chemin, pourquoi}` quand il a l'erreur sous la main. La matière rend la raison
 * telle quelle — elle ne l'invente pas, et ne la remplace pas par une supposition.
 */
const normaliserNonLus = (liste) => (liste || []).map((e) => (typeof e === 'string'
  ? { chemin: e, pourquoi: '' }
  : { chemin: e?.chemin || '', pourquoi: e?.pourquoi || '' }));

/*
 * La raison GROUPE les fichiers, elle ne se répète pas derrière chacun.
 *
 * Une limite d'appels touche six fichiers d'un coup : recopier la même phrase six fois
 * noie le seul fait qui compte — c'est UNE cause, pas six accidents. Groupée, la ligne se
 * lit d'un trait et dit ce qu'il y a à faire.
 */
function ligneNonLus(nonLus) {
  const par = new Map();
  for (const f of nonLus) {
    const cle = f.pourquoi || 'raison inconnue';
    par.set(cle, [...(par.get(cle) || []), f.chemin]);
  }
  const dits = [...par].map(([pourquoi, ou]) => `${pourquoi} : ${ou.join(', ')}`);
  return `  ${nonLus.length} fichier(s) que la lecture N'A PAS PU OBTENIR — ce n'est pas le `
       + `plafond, et ce n'est pas « ils sont vides ».\n${dits.map((d) => `    · ${d}`).join('\n')}`;
}
...
export function analyseBranche({ depot = '', branche = '', brancheDefaut = 'main',
  ...
  const r = {
    ...
    nonLus: normaliserNonLus(nonLus),
...
function texteBranche(r) {
   ...
   * plafond n'y était pour rien — la lecture de ce fichier avait échoué. Le lecteur en
   * conclut qu'il suffit de relever le plafond, ce qui ne changerait rien.
   *
   * `ecartes` est donc ce que le PLAFOND a coupé, une fois les non-obtenus retirés du
  ...
  if (r.nonLus.length) L.push(ligneNonLus(r.nonLus));
  L.push("");
...
function texteDepot(r) {
  ...
  } else if (r.nonLus.length) {
    /*
     * « TOUS LUS » NE SE DIT QUE QUAND C'EST VRAI.
     *
     * L'ancienne version ouvrait le `else` dès que le PLAFOND n'avait rien coupé, et
     * annonçait « tous les fichiers du périmètre ont été lus (21) » deux lignes après un
     * en-tête qui disait « 27 de source dans le périmètre, 21 lu(s) ». Le rapport se
     * contredisait lui-même dans le même paragraphe, et c'est la version rassurante que le
     * modèle a suivie. Un plafond qui n'a rien coupé ne veut pas dire qu'on a tout lu.
     */
    L.push(`  ${r.lus} fichier(s) de source lus sur ${r.candidats} du périmètre. Le plafond`);
    L.push(`  de ${MAX_FICHIERS_DEPOT} n'y est pour rien : le reste n'a pas pu être obtenu.`);
  } else {
...
export default { SIGNAUX_CODE, analyseFichier, resumeCode, analyseBranche, resumeBrancheCode,
                 ...
                 ILLISIBLE, HORS_SOURCE, PAS_DU_CODE,
