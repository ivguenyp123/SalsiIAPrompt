...
/**
 * Retire les secrets d'un texte AVANT qu'il ne parte où que ce soit.
 *
 * ── POURQUOI CETTE FONCTION A DÉMÉNAGÉ ICI ──────────────────────────────────
 *
 * Elle vivait dans `signaux-ci.js`, au motif qu'un log de pipeline est l'endroit où les
 * secrets fuient le plus facilement — un `echo` de débogage, un `curl -v` qui affiche ses
 * en-têtes, un outil qui recopie sa configuration au démarrage. C'est vrai, et le log
 * était donc caviardé avant de partir chez le fournisseur.
 *
 * Mais LE CODE, LUI, PARTAIT EN CLAIR. Un `.env` ouvert dans le sélecteur de fichiers, un
 * `application.yml` avec son mot de passe de base, une clé privée collée dans le champ :
 * tout cela était assemblé dans le prompt et envoyé tel quel. Le contrôle existait, il
 * n'était branché que sur un chemin sur cinq.
 *
 * Le plus troublant est ce que le registre vérifiait à la place. `output.contains_secret`
 * contrôle que la RÉPONSE DU MODÈLE ne contient pas de secret — la porte de sortie. La
 * porte d'entrée n'était gardée nulle part. On surveillait que le modèle ne recopie pas
 * un jeton, sans regarder qu'on venait de le lui donner.
 *
 * Elle est donc ici, à côté des motifs, et le dernier garde-fou avant l'appel s'en sert
 * pour TOUS les agents — voir `runtime/lancer.js`.
 *
 * @returns {{ texte, trouves }} — `trouves` nomme les TYPES rencontrés, jamais les valeurs
 */
export function caviarder(texteBrut = '') {
...
/*
 * Ce qu'on liste en clair. Au-delà, on compte.
 *
 * Déclaré ICI, au-dessus des trois scanners, parce que les trois s'en servent. Il vivait
 * dans la section des dépendances, et la liste des secrets ne le connaissait donc pas :
 * elle sortait ses constats un par un, tous, sans plafond. Un `.tfstate` bavard en produit
 * des dizaines sur la même ligne de motifs, et la matière partait entière.
 *
 * Deux cents lignes ne se lisent pas, ni dans un prompt ni dans un rapport exporté. Ce
 * qu'on tronque est la LISTE ; le compte, lui, reste toujours complet — et le fait d'avoir
 * tronqué est écrit, sinon on lit « 40 secrets » là où il y en a deux cents et le plan
 * d'action est dimensionné pour rien.
 */
...
function texteSecrets({ depot, constats, comptes, parType }) {
  ...
  if (!constats.length) {
    ...
    for (const c of constats.slice(0, MAX_CONSTATS_LISTES)) {
      l.push(`  ${c.fichier}:${c.ligne}  ${c.type.padEnd(34)} ${c.apercu}   CIS ${c.cis}`);
    }
    /*
     * Le plafond est DIT, et il n'y a pas de plafond silencieux dans ce fichier.
     *
     * Un `.tfstate` ou un `docker-compose` bavard déclenche des dizaines de constats sur
     * la même ligne de motifs. Les envoyer tous coûtait une matière énorme pour une
     * information qui se répète — mais les couper sans le dire ferait lire « 40 secrets »
     * là où il y en a deux cents, et le plan d'action serait dimensionné pour rien.
     *
     * Le COMPTE, lui, reste complet : c'est la liste qu'on tronque, jamais la mesure.
     */
    if (constats.length > MAX_CONSTATS_LISTES) {
      l.push(`  … et ${constats.length - MAX_CONSTATS_LISTES} constat(s) de plus, non `
        + `listés ici. Le compte de ${constats.length} plus haut, lui, est complet.`);
    }
    l.push('');
  }
...
function presentationSecrets({ constats, comptes, parType, parFichier }) {
  ...
  if (constats.length) {
    tableaux.push({
      ...
      lignes: constats.slice(0, MAX_CONSTATS_LISTES).map((c) => ({
        ton: 'ko',
        cellules: [{ texte: c.fichier, code: true }, { texte: String(c.ligne) },
                   { texte: c.type }, { texte: c.apercu, code: true }]
      })),
      // Le même plafond que le texte, et pour la même raison : deux cents lignes dans un
      // rapport exporté ne se lisent pas, et la table « par nature » au-dessus porte déjà
      // le compte complet.
      note: 'Les valeurs sont tronquées à huit caractères : assez pour retrouver la ligne, '
          + 'jamais assez pour s\'en servir.'
          + (constats.length > MAX_CONSTATS_LISTES
            ? ` Seuls les ${MAX_CONSTATS_LISTES} premiers sont listés — il y en a `
              + `${constats.length} en tout.`
            : '')
    });
  }
  if (comptes.nonLus > 0) {
  ...
  void parFichier;
  return { sujet: 'Les secrets exposés', entete, tableaux };
}
...
function presentationDependances({ rouges, oranges, comptes, parType }) {
  ...
  return { sujet: 'La chaîne d\'approvisionnement', entete, tableaux };
}
...
function presentationConformite({ controles, ecarts, nonVus, note, verdict, comptes }) {
  ...
  return { sujet: 'La conformité CIS', entete, tableaux };
}
