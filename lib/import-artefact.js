...
import { SOURCES_ENTREES } from './assemblage.js';

/*
 * Le branchement par défaut : la matière se colle à la main. C'est un NOM et pas une
 * chaîne vide, pour que « je n'ai rien choisi » et « j'ai choisi le collage » s'écrivent
 * pareil — il n'y a pas de troisième état.
 */
export const ENTREE_LIBRE = 'matiere';
...
export function refus({ capacite, decisions = {}, outils = [], isolements = [],
  ...
  /*
   * Le branchement d'entrée choisi doit EXISTER au vocabulaire (`SOURCES_ENTREES`,
   * l'autorité unique que L027 consulte aussi). Un nom inventé ferait croire que la
   * plateforme sait aller chercher quelque chose qu'elle ne sait pas chercher — le
   * mensonge exact que le champ `matiere` par défaut existe pour éviter.
   */
  if (decisions.entree && decisions.entree !== ENTREE_LIBRE
      && !SOURCES_ENTREES[decisions.entree]) {
    dit(`L'entrée « ${decisions.entree} » n'est pas au vocabulaire de la plateforme`,
      'Le branchement se choisit dans le vocabulaire des entrées (`lib/assemblage.js`), '
      + 'ou la matière reste collée à la main. Un nom inventé promettrait une matière '
      + 'que la plateforme ne sait pas aller chercher.');
  }
...
export function specDe({ titre = '', corps = '', source = '', chemin = '', iso = null,
  ...
  lignes.push('');
  lignes.push(`Applique la méthode ci-dessus à la matière qu'on te donne, pour : ${titre}`);
  lignes.push('');
  /*
   * ── CES RÈGLES ONT ÉTÉ RECALIBRÉES PAR LA PREMIÈRE EXÉCUTION RÉELLE ────────
   *
   * La version d'origine disait « tu décris ce qu'il faudrait faire, et c'est un humain
   * qui le fait ». Écrite pour interdire l'EXÉCUTION, elle a été lue comme interdisant le
   * LIVRABLE : sur une matière pourtant fournie, le modèle a rendu la procédure au lieu
   * du produit — « voici ce qu'un humain devrait faire » sur trois écrans, et zéro
   * synthèse. Les garde-fous tenaient ; ils étranglaient aussi le travail.
   *
   * La distinction que ces règles portent maintenant : le travail d'ÉCRITURE se fait en
   * entier, l'EXÉCUTION ne se fait jamais, et une étape inexécutable SE DIT et ne bloque
   * pas le reste.
   */
  lignes.push('Règles :');
  lignes.push('');
  lignes.push('- LE TRAVAIL DEMANDÉ EST UN TRAVAIL D\'ÉCRITURE, ET TU LE FAIS EN ENTIER.');
  lignes.push('  Applique la méthode à la matière ci-dessus et produis le RÉSULTAT — pas la');
  lignes.push('  procédure, pas la liste de ce qu\'un humain devrait faire. Si le document');
  lignes.push('  décrit des fichiers à produire, rends leur CONTENU, en sections de texte.');
  lignes.push('- Tu ne lances rien : aucune commande, aucun script, aucun conteneur. Quand une');
  lignes.push('  étape du document exige une exécution — lancer un outil, lire un dépôt,');
  lignes.push('  écrire sur disque — note en UNE ligne que son résultat n\'est pas disponible,');
  lignes.push('  et continue avec ce que la matière contient. L\'indisponible se signale, il');
  lignes.push('  n\'arrête pas le travail.');
  lignes.push('- Ne conclus rien sur ce que la matière ne contient pas, et ne présente jamais');
  lignes.push('  un constat comme VÉRIFIÉ : tu réorganises des observations fournies, tu ne');
  lignes.push('  les as pas contrôlées. « la matière rapporte que… » vaut mieux que « le code');
  lignes.push('  fait… ».');
...
export function versArtefact({ capacite, decisions = {}, corps = '', pack = {},
  ...
  // Le branchement décidé, déjà validé par `refus` : hors vocabulaire, on n'arrive pas ici.
  const entree = decisions.entree && decisions.entree !== ENTREE_LIBRE
    ? decisions.entree : ENTREE_LIBRE;
  ...
  const artefact = {
    ...
    /*
     * LA MATIÈRE : collée à la main par défaut, branchée sur la plateforme par DÉCISION.
     *
     * Par défaut, la variable s'appelle `matiere` — un nom hors vocabulaire, et L027 le
     * dira à raison : une capacité importée n'a aucun signal de la plateforme pour
     * l'alimenter, sa matière se colle à chaque lancement. Faire taire l'avertissement
     * en réutilisant un nom du vocabulaire ferait croire que la plateforme sait aller
     * chercher quelque chose qu'elle ne sait pas chercher.
     *
     * MAIS un humain peut décider le branchement (`decisions.entree`) : « cette méthode
     * s'applique à un fichier — donne-lui `analyse_fichier` ». Le nom vient alors du
     * vocabulaire, sa source aussi (l'autorité est `SOURCES_ENTREES`, jamais la saisie),
     * et le panneau de lancement saura calculer la matière au lieu de la demander. C'est
     * la frontière exacte de L027 : la plateforme ne devine pas, quelqu'un décide.
     */
    variables: [
      { name: entree,
        source: entree === ENTREE_LIBRE ? 'user' : SOURCES_ENTREES[entree],
        required: true,
...
export function enteteDe({ capacite, pack = {}, iso = null, ecr = null, decisions = {},
  ...
  // `entree` : le branchement de la matière sur la plateforme — décidé, comme le reste.
  for (const nom of ['entrees', 'sorties', 'ecrit', 'outils', 'isolement', 'modele', 'entree']) {
...
export default { versArtefact, refus, resteADecider, specDe, enteteDe, normaliserId, AUCUN_OUTIL,
                 ENTREE_LIBRE, NIVEAU_IMPORTE, DOSSIER_IMPORTE, OUVERTURE, CLOTURE, MAX_CORPS };
