...
/** Un corps plus long que ça ne se relit pas en merge request, donc ne se valide pas. */
export const MAX_CORPS = 12000;
...
export function refus({ capacite, decisions = {}, outils = [], isolements = [],
                        ecritures = [], corps = '' } = {}) {
  const out = [];
  const dit = (quoi, detail, bloquant = true) => out.push({ quoi, detail, bloquant });

  for (const c of resteADecider(capacite, decisions)) {
    dit(`« ${c.quoi} » n'est pas renseigné`, c.pourquoi);
  }
  ...
  for (const id of listeDe(decisions.outils)) {
    if (!connus.has(id)) {
  ...
  } else if (iso && iso.applicable === false) {
    dit(`L'isolement « ${iso.titre} » n'est pas applicable aujourd'hui`,
      `${iso.manque || ''} La capacité peut entrer au registre et s'y voir ; elle ne se `
      + 'lancera pas tant qu\'aucun code ne fait respecter cet isolement.', false);
  }
  ...
  if (texte.length > MAX_CORPS) {
    dit(`Le document de l'amont fait ${texte.length} caractères`,
      `Au-delà de ${MAX_CORPS}, il ne se relit pas en merge request — donc il se valide `
...
export function versArtefact({ capacite, decisions = {}, corps = '', pack = {},
  ...
  return { artefact, refus: problemes, entete: enteteDe({ capacite, pack, iso, ecr, decisions }) };
}
...
export function enteteDe({ capacite, pack = {}, iso = null, ecr = null, decisions = {} }) {
  ...
  l.push('#');
  if (iso && iso.applicable === false) {
    l.push('# ⚠ ISOLEMENT NON APPLICABLE — I005.');
    l.push(`# Elle exige « ${iso.titre} », que la plateforme ne sait pas faire respecter.`);
...
export default { versArtefact, refus, resteADecider, specDe, enteteDe, normaliserId,
