...
import { SIGNAUX_LIVRAISON } from './signaux-livraison.js';
import { SIGNAUX_CODE } from './signaux-code.js';
import { SIGNAUX_BRANCHE } from './signaux-branche.js';
import { SIGNAUX_REGIME } from './signaux-regime.js';
import { SIGNAUX_PIPELINES } from './signaux-pipelines.js';
import { SIGNAUX_EXECUTION } from './signaux-execution.js';
import { SIGNAUX_VULNERABILITES } from './signaux-vulnerabilites.js';
...
export const listeDeChoix = (nom) => {
  ...
  /*
   * `execution` et non `run` : la liste `run` ne déroule que les pipelines EN ÉCHEC, ce
   * qui convient à « explique-moi cet échec » et pas du tout à « où passe le temps ».
   * L'exécution qu'on veut ouvrir pour raccourcir une chaîne est le plus souvent une
   * exécution RÉUSSIE — simplement trop longue.
   */
  if (s?.parExecution) return 'execution';
  return null;
};

/**
 * Les RÉGLAGES d'un signal — ce qu'il faut décider avant de pouvoir calculer.
 *
 * ── POURQUOI CE N'EST PAS UNE LISTE DE CHOIX DE PLUS ─────────────────────────
 *
 * `listeDeChoix` répond à « SUR QUOI » : quelle merge request, quel pipeline. L'objet
 * existe déjà dans le dépôt, on le désigne, et sa matière se lit.
 *
 * Un réglage répond à « COMMENT » : quelle branche livrer, vers quel environnement, en
 * majeur, mineur ou correctif. Rien de tout cela ne se lit dans le dépôt — ce sont des
 * INTENTIONS. Les confondre aurait conduit à deviner : livrer la première branche venue en
 * patch, sur tous les environnements, parce qu'il fallait bien afficher quelque chose.
 *
 * Deux d'entre eux sont néanmoins remplis PAR le dépôt — les branches qui existent, les
 * environnements réellement présents dans l'arbre. C'est ce qui sépare un réglage d'un
 * champ libre : on choisit son intention dans ce qui est possible, on ne la tape pas.
 *
 * @returns {Array<{nom, libelle, genre, requis, defaut, options, tous}>}
 */
export const reglagesDe = (nom) => SIGNAUX[String(nom || '')]?.reglages || [];

/** Ce signal demande-t-il des réglages avant de pouvoir calculer ? */
export const seRegle = (nom) => reglagesDe(nom).length > 0;

/**
 * Les réglages requis sont-ils tous posés ?
 *
 * Un réglage facultatif absent est une VALEUR — « tous les environnements » — et pas un
 * trou. Le tester comme un manque bloquerait le calcul sur le cas le plus courant.
 */
export const reglagesComplets = (nom, choisis = {}) =>
  reglagesDe(nom).every((r) => !r.requis || Boolean(choisis[r.nom]));
