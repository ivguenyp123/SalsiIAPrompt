 ...
 *
 * ── ET IL N'EST PAS LE SCORE DE TOUT LE MONDE ────────────────────────────────
 *
 * `score` et « personnes qui couvrent l'essentiel du code » sont la phrase du BUS FACTOR,
 * et d'aucune autre matière. Vu à l'écran sur un rapport de green coding :
 * « undefinedpersonnes qui couvrent l'essentiel du code », parce que `score` valait
 * `undefined` — et `undefined !== null`, donc la branche « non calculable » ne s'ouvrait
 * pas. Une matière qui ne porte pas de score n'a pas un score inconnu : elle n'a pas de
 * score, et un en-tête vide est la seule chose vraie à afficher.
 */
function enTete(m) {
  if (!m || m.score === undefined) return '';
  if (m.score === null) {
...
/*
 * Le sujet d'une matière, en séparateur — et seulement quand il y en a plusieurs.
 *
 * Sur la revue de sécurité, trois scans se suivaient sans rien pour les séparer : deux
 * tableaux s'appelaient « Par nature de secret » et « Par nature », et le lecteur ne
 * pouvait plus dire lequel parlait des secrets et lequel des dépendances. Un rapport dont
 * on ne sait pas d'où vient une ligne ne se conteste pas, donc ne sert à rien.
 *
 * Sur un rapport à une seule matière, le séparateur n'apporterait rien et redirait le
 * titre de la page. Il ne s'affiche donc pas.
 */
const sujetDe = (p, plusieurs) => (plusieurs && p?.sujet
  ? `<h2 class="sujet">${echapper(p.sujet)}</h2>` : '');

function tableauxDecrits(p, plusieurs = false) {
  return sujetDe(p, plusieurs) + (p?.tableaux || []).map((t) => {
    if (!t.lignes?.length) {
 ...
 *
 * `zones` PORTE DEUX SENS DANS LA PLATEFORME, et ce tableau n'en connaît qu'un. Ici c'est
 * `{chemin, facteur, commits, parts}` — le bus factor par répertoire. Dans `code_du_depot`
 * c'est la carte des répertoires, `{zone, fichiers, source}`. Le nom se recouvre, les
 * formes non : le rapport de green coding a rendu six lignes de « undefinedundefined »
 * avant qu'on s'en aperçoive.
 *
 * Le tableau exige donc la forme qu'il sait lire, au lieu de faire confiance au nom.
 */
function tableauZones(m) {
  const zones = (m?.zones || []).filter((z) => z && z.chemin !== undefined
                                            && z.facteur !== undefined);
...
export function rapportHtml(r = {}) {
  const {
    ...
    auteur = '', perimetre = '', mesures = null, motifArret = ''
  } = r;

  const coupe = coupee(motifArret);
  ...
  /*
   * Une réponse coupée le dit DANS le fichier exporté, et pas seulement à l'écran.
   *
   * Le rapport part en pièce jointe et se relit six mois plus tard, par quelqu'un qui
   * n'était pas là au moment du lancement. Un diagnostic qui s'arrête au milieu et dont
   * rien ne signale la coupure se lit comme un diagnostic complet — et c'est la seule
   * chose que ce fichier promet de ne jamais laisser croire.
   */
  const avis = coupe
    ? '<div class="verdict ko">✂ Réponse coupée : elle s\'arrête au plafond de jetons du '
      + 'modèle. Ce qui suit n\'a pas été écrit, et les dernières sections manquent '
      + 'peut-être entièrement. Ne conclus rien de leur absence.</div>'
    : '';

  const corps = avis + (String(sortie).trim()
...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  /* Le séparateur de sujet : il ouvre un groupe, il n'est pas une section de plus. */
  h2.sujet{font-size:15px;font-weight:800;letter-spacing:-.01em;margin:30px 0 14px;
           padding-bottom:10px;border-bottom:2px solid rgba(165,180,252,.4);color:#c7d2fe}
  h2.sujet:first-of-type{margin-top:0}
...
<div class="page">
  ...
  ${decrites.length ? decrites.map((p) => tableauxDecrits(p, decrites.length > 1)).join('') : `
