...
/*
 * LE STOCKAGE DE GIT LUI-MÊME N'EST PAS VERSIONNÉ, ET NE SE COMPTE DONC PAS.
 *
 * Vu en navigateur sur un arbre réel : `.git/logs/HEAD` et `.git/logs/refs/heads/master`
 * remontés en « Journaux et traces ». Ils le sont — mais git ne les suit pas, et proposer
 * de les `.gitignore` est un conseil qui n'a aucun sens.
 *
 * La liste d'arbre d'une forge ne les rend jamais. Un lot de fichiers apporté depuis un
 * poste, si : quelqu'un qui zippe un dossier de travail embarque son `.git` sans y penser.
 * Un relevé qui compte ce que git stocke comme si git le versionnait est faux, et l'excuse
 * « ça n'arrive pas en vrai » vient de tomber.
 */
const HORS_RELEVE = /(?:^|\/)\.git\//;
...
export function familleDe(chemin) {
  if (HORS_RELEVE.test(chemin)) return null;
