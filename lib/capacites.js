 ...
 * conteste.
 *
 * ── DEUX QUESTIONS DIFFÉRENTES, ET LES CONFONDRE VIDE L'ÉCRAN ────────────────
 *
 * `entrees` peut vouloir dire deux choses, et il a fallu le mesurer pour le voir :
 *
 *   UNE OFFRE  « voilà toute la matière que j'ai sous la main ». Un agent qui en réclame
 *              une autre NE PEUT PAS TOURNER : l'écarter est juste. C'est le cas du
 *              panneau de lancement, et c'est le défaut.
 *
 *   UN SUJET   « voilà ce dont je te parle ». Quelqu'un écrit « les vulnérabilités de mon
 *              dépôt » ; un agent qui lit `rapport_vulnerabilites` ET `stack` lui convient
 *              parfaitement — `stack` se lit tout seul dans le dépôt. L'écarter rendait
 *              ZÉRO carte sur une demande à laquelle le registre savait répondre.
 *
 * D'où `couvertureTotale`. En mode sujet, il suffit de lire AU MOINS UNE des matières
 * demandées, et ce que l'agent réclamera en plus est rendu dans `entreesEnPlus` — un coût
 * annoncé, pas une exclusion silencieuse.
 */
export function candidats(fiches = [], { entrees = [], sections = [] } = {},
  ...
  return fiches.map((f) => {
    ...
    return {
      ...
      // En mode sujet : sur quoi il répond, et ce qu'il demandera EN PLUS.
      entreesSurLeSujet: f.entrees.filter((e) => veutE.has(e)),
      entreesEnPlus: manquantes,
      sectionsCouvertes: couvertes,
      sectionsAttendues: veutS.size,
      sectionsEnPlus: f.sections.filter((s) => veutS.size && !veutS.has(s))
    };
  })
    .filter((c) => (couvertureTotale ? !c.entreesManquantes.length
                                     : !veutE.size || c.entreesSurLeSujet.length)
                && (!veutS.size || c.sectionsCouvertes.length))
    .sort((a, b) =>
      (b.sectionsCouvertes.length - a.sectionsCouvertes.length)
      // Sur un sujet, celui qui en couvre le plus passe devant — un compte, pas un score.
      || (b.entreesSurLeSujet.length - a.entreesSurLeSujet.length)
      || (rangMode(b.droit) - rangMode(a.droit))
      // Puis le MOINS exigeant : ce qu'il réclame en plus est du travail pour qui lance.
      || a.entreesEnPlus.length - b.entreesEnPlus.length
