...
export function consigneAssemblee(morceaux = []) {
  ...
  return bouts.map((b, i) => `${i + 1}. ${b}`).join(COUTURE);
}
 ...
 */
export function cequilManque(morceaux = [], { titre = '', purpose = '', scope = '' } = {}) {
  const manques = [];
  if (morceaux.length === 0) manques.push('Prends au moins un prompt à gauche.');
  if (!titre.trim()) manques.push('Donne-lui un titre.');
  if (!purpose.trim()) manques.push('Dis à quoi il sert, en une phrase.');
  if (!scope.trim()) manques.push('Choisis un périmètre : c\'est lui qui décide des outils permis.');
  return manques;
}

export default { SOURCES_ENTREES, CRITERE_PAR_SORTIE, COUTURE, consigneDepuisBesoin,
