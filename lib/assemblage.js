...
export const SOURCES_ENTREES = {
  ...
  /*
   * Ce qu'un dépôt versionne et ne devrait pas — et c'est encore une autre question.
   *
   * `code_du_depot` lit le CODE et écarte tout le reste. `regime_du_depot` fait l'inverse :
   * il ne lit rien du tout, et ne regarde QUE ce que le code n'est pas — binaires livrés,
   * archives, dossiers de dépendances, clés. Deux signaux, parce que « comment ce dépôt
   * est-il fait » et « qu'est-ce qu'il traîne » ne se répondent pas ensemble.
   */
  regime_du_depot: 'signal',

  // Ce que quelqu'un tape.
  besoin_metier: 'user',
  requete: 'user',
  /*
   * UN LOT DE FICHIERS APPORTÉ DEPUIS UN POSTE — `user`, et c'est exact.
   *
   * Ce n'est ni un signal (la plateforme ne le calcule pas) ni une lecture de dépôt
   * (elle ne va pas le chercher) : quelqu'un l'apporte. Des rapports exportés, un dossier
   * de CI qui vit sur une forge qu'on ne joint pas encore. La source dit donc la vérité
   * sur ce que ça vaut : contrairement à tout le reste de la matière, personne d'autre ne
   * peut le relire — et le texte assemblé porte cet avertissement en tête.
   */
  lot_de_fichiers: 'user',
  story: 'user',
  notes_incident: 'user'
};
