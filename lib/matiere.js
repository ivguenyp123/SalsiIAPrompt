...
export const SOURCES = [
  ...
  /*
   * DES FICHIERS DU POSTE — la seule source qui ne vient pas d'une forge.
   *
   * Les trois autres tirent d'un dépôt : contestable, rejouable, relisible par quelqu'un
   * d'autre. Celle-ci non, et c'est assumé — le besoin est venu de l'usage : on a
   * quatorze rapports exportés sur son poste, ou un dossier de CI qui vit sur une forge
   * que la plateforme ne joint pas encore. Sans elle, le geste est « ouvre les quatorze
   * fichiers et colle-les à la suite », ce qui est un travail, pas un geste.
   *
   * En DERNIER dans la liste, après le collage : ce n'est pas la voie normale, et le
   * texte produit porte son en-tête de provenance pour que ça se voie jusqu'au bout.
   */
  { id: 'poste', icone: '🗂️', libelle: 'Des fichiers de mon poste',
    aide: 'Un lot de fichiers, ou une archive ZIP. Chacun garde son nom dans la matière. '
 ...
 * encore, comme une entrée sortie de Composer — reçoit la zone. Le défaut penche du côté
